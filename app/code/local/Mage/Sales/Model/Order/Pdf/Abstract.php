...
abstract class Mage_Sales_Model_Order_Pdf_Abstract extends Varien_Object
{
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
        ...
        if ($putOrderId) {
            // ORDER NUMBER
            $page->drawText(Mage::helper('sales')->__('').$order->getRealOrderId(), 270, 585, 'UTF-8');
            // copy on the other side
            $page->drawText(Mage::helper('sales')->__('').$order->getRealOrderId(), 689, 585, 'UTF-8');
        }
        //$page->drawText(Mage::helper('sales')->__('Order Date: ') . date( 'D M j Y', strtotime( $order->getCreatedAt() ) ), 35, 760, 'UTF-8');
        // Order Date 
        $page->drawText(Mage::helper('sales')->__('') . Mage::helper('core')->formatDate($order->getCreatedAtStoreDate(), 'medium', false), 270, 600, 'UTF-8');
        // copy on the other side
        $page->drawText(Mage::helper('sales')->__('') . Mage::helper('core')->formatDate($order->getCreatedAtStoreDate(), 'medium', false), 689, 600, 'UTF-8');
        ...
        // BILLING ADDRESS / ORDERED BY 
        foreach ($billingAddress as $value){
            if ($value!=='') {
                $page->drawText(strip_tags(ltrim($value)), 0, $this->y, 'UTF-8');
                // copy on the other side
                $page->drawText(strip_tags(ltrim($value)), 419, $this->y, 'UTF-8');
                $this->y -=10;
            }
        }

        if (!$order->getIsVirtual()) {
            // SHIP ADDRESS IS HERE
            $this->y = 530;
            foreach ($shippingAddress as $value){
                if ($value!=='') {
                    $page->drawText(strip_tags(ltrim($value)), 190, $this->y, 'UTF-8');
                    // copy on the other side
                    $page->drawText(strip_tags(ltrim($value)), 609, $this->y, 'UTF-8');
                    $this->y -=10;
                }
    ...
    public function drawLineBlocks(Zend_Pdf_Page $page, array $draw, array $pageSettings = array())
    {
        foreach ($draw as $itemsProp) {
            ...
            // this is where the new page kicks in 
