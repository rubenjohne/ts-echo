    ...
    public function printAction()
    {
        if ($invoiceId = $this->getRequest()->getParam('invoice_id')) {
            if ($invoice = Mage::getModel('sales/order_invoice')->load($invoiceId)) {
                ...
                // i need to get the order here to add the customer name
                $order = Mage::getModel('sales/order')->load($invoice->getOrderId());
                $this->_prepareDownloadResponse($order->getCustomerName().'-'.$order->getRealOrderId().'-'.Mage::getSingleton('core/date')->date('Y-m-d_H-i-s').
