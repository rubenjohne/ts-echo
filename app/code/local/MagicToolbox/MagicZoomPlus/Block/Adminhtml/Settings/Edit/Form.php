...
class MagicToolbox_MagicZoomPlus_Block_Adminhtml_Settings_Edit_Form extends Mage_Adminhtml_Block_Widget_Form {
    ...
    protected function _prepareForm() {
        ...
        $model = Mage::registry('magiczoomplus_data');
        $data = $model->getData();
        ...
        foreach($this->getParamsMap() as $group => $ids) {
            $fieldset = $form->addFieldset('group'.$gId++, array('legend'=>Mage::helper('magiczoomplus')->__($group), 'class'=>'magiczoomplusFieldset'));
            foreach($ids as $id) {
                if($tool->type == 'standard' && $id == 'direction') continue;
                $config = array(
                    'label'     => Mage::helper('magiczoomplus')->__($tool->params->params[$id]['label']),
                    'name'      => $id,
                    'note'      => '',
